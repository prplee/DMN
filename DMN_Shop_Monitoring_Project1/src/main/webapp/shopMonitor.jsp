...
$(document).ready(function () {
	 ...
	 $('#complete_y').on("click",function(){  
		 ...
		 if(chb == false) {
			 swal("주문을 선택하세요.", "You clicked the button!", "warning");
	 		return;
		 }
 	...
 	$("#pickupCall").on("click",function(){
 		...
 		$("input:checkbox[name=orderChk]").each(function() {
			...
			if(num == 0 ) {
				swal("호출할 주문을 선택하세요.", "You clicked the button!", "warning");
			} else if (num > 1) {
				swal("체크박스를 한개만 선택하세요", "You clicked the button!", "error");
			} else {
				swal("주문번호가 호출되었습니다.", "You clicked the button!", "success");
	 ...
	 $('#cancel').on("click", function () {
		 ...
		 if(chb == false) {
			 swal("주문취소항목을 선택하세요.", "You clicked the button!", "warning");
	 		return;
		 }
...
<div id="origin"> <!-- 전체테두리  -->
			<form id="formMode" action="SaleListServlet">
				<input type="hidden" id="YN" value="" name="YN">
			</form>
